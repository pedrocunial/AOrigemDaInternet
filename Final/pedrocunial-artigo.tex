...
% affiliations
\author{\IEEEauthorblockN{Pedro Cunial}
	\IEEEauthorblockA{Insper - Instituto de Ensino e Pesquisa\\
		São Paulo, SP\\
...
% in the abstract
\begin{abstract}
	Este artigo busca descrever o processo da criação da Internet em seu primórdio, almejando compreender o motivo dos formatos atuais pelos quais compartilhamos dados pela óptica das telecomunicações.
\end{abstract}
...
% no \IEEEPARstart
  ...
  Em 1962 a Agência de Projetos de Pesquisa Avançada (ARPA) criaria um novo
  braço, o Escritório de Técnicas de Processamento de Informações (IPTO), o qual se
  ...
  Mas fora em 1966, quando Robert Taylor iniciou seus estudos em redes que a
  Internet seria idealizada pela primeira vez. A proposta de Taylor consistia em
  suprir duas necessidades: A redução de custos em
  computação pelo compartilhamento de computadores por contratantes ao redor do
  país e o avançar na transferência de informações entre máquinas à
  distância~\cite{fromarpanet}.
...
%	Neste parágrafo pretendo explicar os processos iniciais de ideação, anteriores às primeiras versões da mesma. Pretendo basear este parágrafo fortemente na descrição do período pelo artigo "From ARPANET to Internet"~\cite{fromarpanet}, prendendo-me aos períodos anteriores ao ano de 1969, quando a primeira versão da ARPANET for publicada.
  \begin{quote}
    Quando o Departamento de Defesa (DOD) emitiu um contrato de \$19.800 em
  ...
  Pela confidencialidade do projeto, não sabemos muito sobre o que ocorrera de
  fato nestes primeiros anos, apenas que em 1968 mais \$63.000 seriam investidos
  no projeto, o qual teria sua segunda versão liberada em 1969,
  \say{com uma revolução nas comunicações pelas palavras
    ``log in''}~\cite{fromarpanet}.
...
\section{Primeira versão}
  ...
  No entanto, ainda nesta segunda versão, não existiam padrões no formato das
  comunicações, gerando um verdadeiro caos, onde por mais que mensagens fossem
  enviadas, poucas conseguiam de fato ser decodificadas dada esta não
  padronização oficial.

  Buscando solucionar o problema, foi criado o padrão dos Request For Comments
  (RFC) em 1969 por Steve Crocker e atualmente mantidos pela Internet
  Engineering Task Force (IETF), como documentado no RFC 100:
  ...
  redes (RFC 356), ``netiqueta'' (RFC 1855)~\cite{framingyears}. Hoje os
  documentos são considerados não só protocolos, mas também documentos
  acadêmicos, que vão desdo ramo da ciência da computação até as ciências sociais.

\section{Surgimento do TCP}
...
\section{TCP/IP e como ele fora adotado como padrão}
	Aqui pretendo expandir no conceito dos RFC's iniciais e como o formato original fora substituído pelo modelo TCP/IP. Sinceramente, ainda não tenho muita certeza se devo explicar o conceito de IP e a união com o protocolo TCP. De qualquer forma, o "Framing Years"~\cite{framingyears} fala mais sobre esta transição do qe os outros artigos, por isso acredito que ele seja uma referência melhor.
